...
def get_neg_samples_replacing_head_tail(test_links, adj_list, num_samples=50):
    ...
    for i, (head, tail, rel) in enumerate(zip(heads, tails, rels)):
        ...
        neg_triplet['head'][0].append([head, tail, rel])  # 正
        #
...
def get_subgraphs(all_links, adj_list, dgl_adj_list, max_node_label_value, id2entity, node_features=None,
                  kge_entity2id=None):
...
def get_rank(neg_links):
    ...
    if head_target_id != 10000:
        data = get_subgraphs(head_neg_links, adj_list_, dgl_adj_list_, model_.max_label_value, id2entity_,
                             node_features_, kge_entity2id_)
        head_scores = model_(data[0], model_.parameters()).squeeze(1).detach().cpu().numpy()
    ...
    if tail_target_id != 10000:
        data = get_subgraphs(tail_neg_links, adj_list_, dgl_adj_list_, model_.max_label_value, id2entity_,
                             node_features_, kge_entity2id_)
        tail_scores = model_(data[0], model_.parameters()).squeeze(1).detach().cpu().numpy()
...
def main(params):
    ...
    for link in tqdm(neg_triplets, total=len(neg_triplets)):  # 'head','tail'
...
if __name__ == '__main__':
    ...
    parser.add_argument("--experiment_name", "-e", type=str, default="WN18RR_v2",
                        help="Experiment name. Log file with this name will be created")
    parser.add_argument("--dataset", "-d", type=str, default="WN18RR_v2_ind",
    ...
    file_handler = logging.FileHandler(
        os.path.join('experiments', params.experiment_name, f'log_rank_test.txt'))

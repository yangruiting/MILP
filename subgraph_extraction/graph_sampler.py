...
from utils.graph_utils import incidence_matrix, remove_nodes, ssp_to_torch, serialize, deserialize, get_edge_count, \
    diameter, radius
...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    enc_ratios = []
    num_pruned_nodes = []
    sum_BYTES_PER_DATUM = 0
    split_list = ['one', 'one2', 'many', 'many2']
    for i in range(4):
        sum_BYTES_PER_DATUM = sum_BYTES_PER_DATUM + get_average_subgraph_size(100,
                                                                              list(graphs[split_list[i]].values())[0][
                                                                                  'pos'], A, params) * 1.5
    BYTES_PER_DATUM = sum_BYTES_PER_DATUM / 4

    links_length = 0
    for split_name, split in graphs.items():
        links_length += (len(split['train']['pos']) + len(split['train']['neg']) + len(split['valid']['pos']) + len(
            split['valid']['neg'])) * 2
    map_size = links_length * BYTES_PER_DATUM
    env = lmdb.open(params.db_path, map_size=2*map_size, max_dbs=24)
    ...
    for split_name, split in graphs.items():
        ...
        labels_train = np.ones(len(split['train']['pos']))
        labels_valid = np.ones(len(split['valid']['pos']))
        db_name_pos_train = split_name + '_train' + '_pos'
        db_name_pos_valid = split_name + '_valid' + '_pos'
        split_env_train = env.open_db(db_name_pos_train.encode())
        split_env_valid = env.open_db(db_name_pos_valid.encode())
        extraction_helper(A, split['train']['pos'], labels_train, split_env_train)
        extraction_helper(A, split['valid']['pos'], labels_valid, split_env_valid)

        logging.info(f"Extracting enclosing subgraphs for negative links in {split_name} set")
        labels_train = np.zeros(len(split['train']['neg']))
        labels_valid = np.zeros(len(split['valid']['neg']))
        db_name_neg_train = split_name + '_train' + '_neg'
        db_name_neg_valid = split_name + '_valid' + '_neg'
        split_env_train = env.open_db(db_name_neg_train.encode())
        split_env_valid = env.open_db(db_name_neg_valid.encode())
        extraction_helper(A, split['train']['neg'], labels_train, split_env_train)
        extraction_helper(A, split['valid']['neg'], labels_train, split_env_valid)
...
def links2subgraphs_test(A, graphs, params, max_label_value=None):
    '''
    extract enclosing subgraphs, write map mode + named dbs
    '''
    max_n_label = {'value': np.array([0, 0])}
...
def subgraph_extraction_labeling(ind, rel, A_list, h=1, enclosing_sub_graph=True, max_nodes_per_hop=None,
                                 max_node_label_value=None):
    A_incidence = incidence_matrix(A_list)
    A_incidence_rev = A_incidence.T
    root1_nei = get_neighbor_nodes(set([ind[0]]), A_incidence, h, max_nodes_per_hop)
    root2_nei = get_neighbor_nodes(set([ind[1]]), A_incidence_rev, h, max_nodes_per_hop)

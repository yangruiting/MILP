...
from utils.data_utils import process_files, process_files_test, save_to_file, plot_rel_dist
from .graph_sampler import *
import pdb
import torch

def generate_subgraph_datasets(params, splits=['one', 'one2', 'many', 'many2'], saved_relation2id=None,
                               max_label_value=None):
    testing = 'test' in splits
    adj_list, triplets_S_Q, entity2id, relation2id, id2entity, id2relation = process_files(params.file_paths,
                                                                                           saved_relation2id)
    ...
    one2one['train'] = {'triplets': triplets_S_Q['one']['train'], 'max_size': params.max_links}
    one2one['valid'] = {'triplets': triplets_S_Q['one']['valid'], 'max_size': params.max_links}
    graphs['one'] = one2one
    one2many['train'] = {'triplets': triplets_S_Q['one2']['train'], 'max_size': params.max_links}
    one2many['valid'] = {'triplets': triplets_S_Q['one2']['valid'], 'max_size': params.max_links}
    graphs['one2'] = one2many
    many2one['train'] = {'triplets': triplets_S_Q['many']['train'], 'max_size': params.max_links}
    many2one['valid'] = {'triplets': triplets_S_Q['many']['valid'], 'max_size': params.max_links}
    graphs['many'] = many2one
    many2many['train'] = {'triplets': triplets_S_Q['many2']['train'], 'max_size': params.max_links}
    many2many['valid'] = {'triplets': triplets_S_Q['many2']['valid'], 'max_size': params.max_links}
    graphs['many2'] = many2many
    for split_name, split in graphs.items():
        for split_name_inner, split_inner in split.items():
            logging.info(f"Sampling negative links for {split_name}")
            height = len(split_inner['triplets'])
            split_inner['pos'], split_inner['neg'] = sample_neg(adj_list, split_inner['triplets'],
                                                                params.num_neg_samples_per_link,
                                                                max_size=split_inner['max_size'],
...
def generate_subgraph_datasets_test(params, splits=['test'], saved_relation2id=None, max_label_value=None):
    testing = 'test' in splits
    adj_list, triplets, entity2id, relation2id, id2entity, id2relation = process_files_test(params.file_paths,
                                                                                            saved_relation2id)
    ...
    links2subgraphs_test(adj_list, graphs, params, max_label_value)
...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None,
        ...
        self.main_env = lmdb.open(db_path, readonly=False, max_dbs=3, lock=False)
        ...
        ssp_graph, __, __, __, id2entity, id2relation = process_files_test(raw_data_paths, included_relations)
